...
#/var/www/serikov/data/www/backups/redmine.fablab61.ru/redmine-master_'.mysql_now().'.meta.txt';
my $filename = '>meta.txt';
my $db = "ru_fablab61_redmine";
...
sub have_id_and_created {
	open FILE, $filename;
	print FILE "# Meta info about last fields in db with target columns:\n";
	...
	print FILE "# Full list of tables:\n";
	for (@tables) {
		my ($db, $table) =  split '\.', $_ ;
		$table =~ s/\`//g;
		print FILE $table."\n";
